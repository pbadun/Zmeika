...
import com.example.gm3.framework.Pixmap;
import com.example.gm3.framework.Screen;
import com.example.gm3.zmeika.Assets;
import com.example.gm3.zmeika.Settings;
import com.example.gm3.zmeika.games.Snake;
import com.example.gm3.zmeika.games.SnakePart;
import com.example.gm3.zmeika.games.Stain;
import com.example.gm3.zmeika.games.World;

public class GameScreen extends Screen {
	enum GameState {
		Ready, Running, Pauserd, GameOver
	}
	...
	public void update(float deltaTime) {
		...
		if (state == GameState.Pauserd) {
			updatePause(touchEvent);
		}
	...
	/**
	 * Процесс обработки игры
	 * 
	 * @param touchEvent
	 * @param deltaTime
	 */
	private void updateRunning(List<TouchEvent> touchEvent, float deltaTime) {
	...
	private void updatePause(List<TouchEvent> touchEvent) {
		int len = touchEvent.size();
		for (int i = 0; i < len; i++) {
			TouchEvent event = touchEvent.get(i);
			if (event.type == TouchEvent.TOUCH_UP) {
	...
	private void updateGameOver(List<TouchEvent> touchEvent) {
		int len = touchEvent.size();
		for (int i = 0; i < len; i++) {
			TouchEvent event = touchEvent.get(i);
			if (event.type == TouchEvent.TOUCH_UP) {
